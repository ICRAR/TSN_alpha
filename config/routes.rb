Tsn::Application.routes.draw do
  ...
  match "/(*path)" => redirect {|params, req| "#{req.protocol}#{APP_CONFIG['site_host']}:#{req.port}/#{params[:path]}"},
        constraints: lambda{|request| (request.host != APP_CONFIG['site_host'] && request.host !=' localhost')}
